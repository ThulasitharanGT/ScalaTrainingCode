...
// method misses arguments for its implicit parameters, such arguments will be automatically provided.
/** This example uses a structure from abstract algebra to show how implicit parameters work.
  * A semigroup is an algebraic structure on a set A with an (associative) operation, called add here,
  * that combines a pair of A's and returns another A. */
abstract class SemiGroup[A] {
  def add(x: A, y: A): A
}
/** A monoid is a semigroup with a distinguished element of A, called unit, that when
  * combined with any other element of A returns that other element again. */
abstract class Monoid[A] extends SemiGroup[A] {
  def unit: A
}
object ImplicitParameters_30 extends App {
